...
const url = 'https://unionserver.herokuapp.com/posts/latest';
...
request$.pipe(
  ...
  concatMap((posts) => { // последовательно переключаем поток
    const comments$ = posts.map((post) => ajax.getJSON(`https://unionserver.herokuapp.com/posts/${post.id}/comments/latest`).pipe(// создаём новый потом для получения коментариев к определённому посту
